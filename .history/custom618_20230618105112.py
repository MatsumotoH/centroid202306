...
class VideoStream:
    """Camera object that controls video streaming from the IPcamera"""

    def __init__(self, resolution=(2560, 1440), framerate=30):
...
while True:
    ...
    for i in range(len(scores)):
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
            ...
            # object_name 'car' or 'truck' or 'bus'
    ...
    if obsFrames % 1 == 0:
        ...
        if ymax <= 1000 and xmax > 550 and xmin > 525:
            d1 = {}
            if objectID not in d1:
                for k, v in x.items():
                    ...
                    if bool(d1):
                        ...
                        # Dのリストの内容をframeに書き込む
                        for k, v in d.items():
                            cv2.putText(frame, v, (xmin, ymin),
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)

                    # room 2
        if ymax > 1000 and xmin < 1000:
            d2 = {}
            if objectID not in d2:
                for k, v in x.items():
                    if v[0] > 25:
                        d2[k] = "ROOM2OUT"
                        room2.append(v[0])
        ...
        # go2Recordsというファイルに入庫、出庫の記録を残す
                        with open('go2Records.txt', 'a', encoding='utf-8') as f:
                            print(d2, objectID, xmin, ymin, xmax, ymax,
                                  v[0], time.ctime(), file=f)
                        with open('go2Records2.txt', 'a', encoding='utf-8') as f2:
